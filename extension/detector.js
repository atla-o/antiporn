...
function detectNudity(image, severity, sourceWidth, sourceHeight) {
  ...
  for (const c of clusters) {
    ...
    boxes.push({ x: mx - sizePx / 2, y: my - sizePx / 2, size: sizePx, kind: force || hardScore > 0.55 ? "explicit" : "skin" });
  }
...
function rasterFromElement(el, maxWidth) {
  ...
  try {
    ...
    return { image: ctx.getImageData(0, 0, w, h), width: el.clientWidth || sw, height: el.clientHeight || sh };
  } catch {
    return null;
  }
}
